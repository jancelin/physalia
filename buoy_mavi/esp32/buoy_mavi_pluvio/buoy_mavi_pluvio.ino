...
void printPVTdata(UBX_NAV_PVT_data_t *ubxDataStruct)
{
  ...
  vardate = "'"+date1+time1+"'"; //for PLUVIO
...
void setup()
{
  ...
  pinMode(rainPin, INPUT);          // Montage PullUp avec Condensateur pour éviter l'éffet rebond.
...
void getSendRain() {
  ...
  // allocate the memory for the document
  StaticJsonDocument<256> docpluvio;
  // create an object
  JsonObject object = docpluvio.to<JsonObject>();

  docpluvio["capteur"] = matUuid + WiFi.macAddress()+"'";//matUuid; // Print capteur uuid
  object["datetime"] = vardate; // print date time for postgresql data injection.

  double pluvio = pluie; // Print the pluvio
  docpluvio["pluvio"] = round(pluvio*100)/100; //arondi à 2 décimal

  serializeJson(docpluvio, Serial);
  Serial.println();

  //Send position to MQTT broker
  String msg;
  String output = "JSON = ";
  serializeJson(docpluvio, msg);
  client.publish(mqtttopic, msg.c_str());
  Serial.println("Message pluvio send");
}
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
